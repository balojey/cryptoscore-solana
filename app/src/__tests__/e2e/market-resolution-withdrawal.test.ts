...
import { MatchOutcome, MarketStatus } from '../../types/solana-program-types'
      ...
      try {
        ...
        if (accountInfo && accountInfo.data.length > 0) {
          ...
          if (market.status === MarketStatus.Resolved) {
            ...
            let winnerCount = 0
            if (market.outcome === MatchOutcome.Home) winnerCount = market.homeCount
            else if (market.outcome === MatchOutcome.Draw) winnerCount = market.drawCount
            else if (market.outcome === MatchOutcome.Away) winnerCount = market.awayCount
